#!/usr/bin/env python3
"""
Test runner for Problem 2 Round 1 algorithms.
...
ALGORITHMS = [
    ("mikael_func.py", "Mikael"),
    ("chatgpt_4.py", "ChatGPT 4"),
    ("gemini_3.py", "Gemini 3 Pro"),
    ("opus_4.5.py", "Claude Opus 4.5"),
    # ("chatgpt_5.1.py", "ChatGPT 5.1"),  # FAILED - excluded
]
...
def run_algorithm(filename):
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=600  # 10 minute timeout for this harder problem
        )
...
def main():
    ...
    for filename, name in ALGORITHMS:
        ...
        for run in range(1, NUM_RUNS + 1):
            ...
            if success and exec_time is not None:
                ...
                # Get last line of output for display
                last_line = output.split('\n')[-1] if output else ""
                print(f"{exec_time:.2f}s - Output: {last_line[:60]}...")
            else:
                print(f"FAILED - {output[:100] if output else 'No output'}")
    ...
    for name, data in sorted_results:
        if data['average']:
            print(f"{name:<20} {data['average']:<15.2f} {data['min']:<12.2f} {data['max']:<12.2f} {data['successful_runs']:<8}")
        else:
